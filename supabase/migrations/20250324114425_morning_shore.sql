/*
  ...
  1. Changes
    ...
    - Use current_user instead of auth.uid()
...
-- Create new policies with simplified admin checks
CREATE POLICY "commission_rules_read_v7"
  ON commission_rules
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "commission_rules_insert_v7"
  ON commission_rules
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
CREATE POLICY "commission_rules_update_v7"
  ON commission_rules
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY "commission_rules_delete_v7"
  ON commission_rules
  FOR DELETE
  TO authenticated
  USING (
