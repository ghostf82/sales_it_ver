/*
  # Fix update_user_roles function with unique name
  
  1. Changes
    - Create a new function with a different name to avoid ambiguity
    - Maintain the same functionality as the original function
    - Ensure proper parameter handling
    
  2. Security
    - Maintain SECURITY DEFINER setting
    - Preserve super admin access checks
    - Ensure proper error handling
*/

-- Drop the existing function if it exists (all versions)
...
-- Create a new function with a different name to avoid ambiguity
CREATE OR REPLACE FUNCTION update_user_roles_v2(
  p_user_id uuid,
  p_is_admin boolean,
