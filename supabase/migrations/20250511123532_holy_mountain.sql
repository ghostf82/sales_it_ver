/*
  # Fix financial audit trigger stack depth limit error
  
  1. Changes
    - Fix the handle_financial_audit_update function to prevent infinite recursion
    - Add proper NULL handling for deduction fields
    - Only set updated_at if it's not already set by the client
    
  2. Security
    - Maintain existing security checks
    - Preserve admin and financial auditor access control
*/
