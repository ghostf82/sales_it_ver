...
-- Create or replace the net_sales_per_category_view with explicit type casting
...
JOIN financial_audit fa
  ...
  AND rd.year = fa.year
  AND CAST(rd.month AS TEXT) = CAST(fa.month AS TEXT); -- Explicit type cast to ensure both sides are text

-- Add security to the view
ALTER VIEW net_sales_per_category_view OWNER TO postgres;
