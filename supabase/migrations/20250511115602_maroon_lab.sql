...
BEGIN
  ...
  WITH financial_audits AS (
    SELECT DISTINCT
      ...
      fa.company_id,
      fa.year,
      fa.month::integer  -- Cast month to integer for comparison
    FROM financial_audit fa
    WHERE fa.year = p_year AND fa.month::integer = p_month  -- Cast month to integer for comparison
  )
  SELECT 
...
JOIN financial_audit fa
  ...
  AND rd.year = fa.year
  AND rd.month::text = fa.month;  -- Cast integer to text for comparison