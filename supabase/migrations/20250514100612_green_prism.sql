/*
  # Fix financial audit policies and add salesperson_name column
  
  1. Changes
    - Drop existing policies if they exist to avoid conflicts
    - Create new policies with auth.uid() instead of uid()
    - Add salesperson_name column to financial_audit table
    - Add trigger to update updated_at field
    
  2. Security
    - Allow admin users to read financial audit data
    - Allow super admin users to read all financial audit data
