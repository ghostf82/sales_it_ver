...
if (!DISABLED && process.env.SUPABASE_URL && process.env.SUPABASE_KEY) {
  ...
  // No-op client btu keep callers safe when disabled or misconfigured
  supabase = {
    from() {
      return {
        select: async () => ({ data: null, error: new Error('SUPABASE_DISABLED') }),
        insert: async () => ({ data: null, error: new Error('SUPABASE_DISABLED') }),
        update: async () => ({ data: null, error: new Error('SUPABASE_DISABLED') }),
...
umodule.exports = { supabase, disabled: DISABLED };
